...
void HttpServer::InitHttpRouter(const std::shared_ptr<Http::HttpSession> &pSession)
{
    ...
    pSession->AddRouter(Verb::get, "/",
                        [](const Http::HttpRequest &request) -> std::optional<Http::HttpResponse>
                        {
                            ...
                            resp.SetHeader(Http::Field::content_type, "text/html;charset=utf-8bom");
    ...
    pSession->AddRouter(Verb::get, "/test",
                        [](const Http::HttpRequest &request) -> Http::HttpResponse
                        {
                            Http::HttpResponse resp;
                            g_LoginDatabase.AsyncExecute("insert into account(email, name, age) values('123@qq.com', 'tom', 20)");
                            // if (nullptr == pResult)
                            // {
    ...
    pSession->AddRouter(Verb::get, "/async",
                        [this](const Http::HttpRequest &request) -> Http::HttpResponse
                        {
                            Http::HttpResponse resp;
                            _processor.AddCallback(g_LoginDatabase.AsyncQuery("select id, name, age, intro from account where email='123@qq.com'"))
                                .Then(
                                    [&resp](const ResultSetPtr &pResult) mutable
                                    {
                                        // Field *pFields = pResult->Fetch();
                                        // int         id      = pFields[0];
                                        ...
                                        if (nullptr == pResult)
                                        {
                                            ...
                                            // Log::Debug("{}", "查询失败");
                                            resp.SetStatusCode(Status::internal_server_error);
                                            resp.SetHeader(Http::Field::content_type, "text/html;charset=utf-8");
                                        ...
                                        else
                                        {
                                            ...
                                            Field *pFields = pResult->Fetch();
                                            Log::Debug("{}", std::format("id:{}, name:{}, age:{}, intro = {}", pFields[0].operator uint32_t(), pFields[1].operator std::string(), pFields[2].operator uint32_t(), pFields[3].operator std::string()));
                                            resp.SetContent(std::format("id:{}, name:{}, age:{}, intro = {}", pFields[0].operator uint32_t(), pFields[1].operator std::string(), pFields[2].operator uint32_t(), pFields[3].operator std::string()));
                                        }
                                    });
    ...
    // Http::HttpResponse resp;
    // g_LoginDatabase.AsyncQuery("select id, name, age, intro from account where email='123@qq.com'")
    //     .Then(
    //         [&resp](ResultSetPtr pResult)
    //         {
    //             Log::Debug("{},{}", "Then expersion", resp.GetPayload());
    //             Field *pFields = pResult->Fetch();
    //             Log::Debug("{}", std::format("id:{}, name:{}, age:{}, intro = {}", pFields[0].operator uint32_t(), pFields[1].operator std::string(), pFields[2].operator uint32_t(), pFields[3].operator std::string()));
    //             resp.SetStatusCode(Status::ok);
    //             if (nullptr == pResult)
