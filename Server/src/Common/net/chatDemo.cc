...
class ProtoMessage : public std::enable_shared_from_this<ProtoMessage>
{
public:
    enum
    {
...
class ChatRoomParticipant : public std::enable_shared_from_this<ChatRoomParticipant>
{
public:
    virtual ~ChatRoomParticipant()                                 = default;
    virtual void deliver(const std::shared_ptr<ProtoMessage> &msg) = 0;
};
...
public:
    ...
    void deliver(const std::shared_ptr<ProtoMessage> &msg)
    {
        ...
        {
            asio::post(participant.get()->shared_from_this(), [participant, msg]()
                       { participant->deliver(msg); });
        }
    }
...
private:
    ...
    std::deque<std::shared_ptr<ProtoMessage>>  recent_msgs_;
...
public:
    ...
    void deliver(const std::shared_ptr<ProtoMessage> &msg) override
    {
...
private:
    ...
    void do_read_body()
    {
        ...
        asio::async_read(socket_, asio::buffer(read_msg_.body(), read_msg_.length()),
                         [this, self](std::error_code ec, std::size_t /*length*/)
                         {
                             if (!ec)
                             {
                                 std::shared_ptr<google::protobuf::Message> message(read_msg_.get_message());
                                 if (message)
                                 {
                                     room_.deliver(std::make_shared<ProtoMessage>(*message));
                                 }
                                 do_read_header();
                             }
                             else
    ...
    {
        auto self(shared_from_this());
        asio::async_write(socket_, asio::buffer(write_msgs_.front().get()->data(), write_msgs_.front().get()->length()),
    ...
    std::deque<std::shared_ptr<ProtoMessage>> write_msgs_;
};
