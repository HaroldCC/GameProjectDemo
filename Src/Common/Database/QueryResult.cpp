...
static constexpr const char *FieldTypeToString(enum_field_types type, uint32_t flags)
{
    switch (type)
    {
        case MYSQL_TYPE_BIT:
...
static constexpr void InitializeDatabaseFieldMetadata(QueryResultFieldMetadata *meta,
...
PreparedResultSet::PreparedResultSet(MySqlStmt   *pStmt,
                                     MySqlResult *pResult,
                                     uint64_t     rowCount,
                                     uint32_t     fieldCount)
    : _rowCount(rowCount)
