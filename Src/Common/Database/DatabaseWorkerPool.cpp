...
#include "DatabaseImpl/LoginDatabase.h"
#include "Common/Util/Util.hpp"

template <typename ConnectionType>
DatabaseWorkerPool<ConnectionType>::DatabaseWorkerPool() : _queue(new ProducerConsumerQueue<ISqlTask *>())
{
                                                  ...
                                                  uint8_t                    asyncThreadCount)
{
    ...
    if (0 == errcode)
    {
        Log::Info("连接数据库：{} 成功。当前共{}个连接",
                  info._database,
                  (_connections[EConnectionTypeIndex_Sync].size()
                   + _connections[EConnectionTypeIndex_Async].size()));
    }
...
template <typename ConnectionType>
TransactionCallback
DatabaseWorkerPool<ConnectionType>::CommitTransactionWithCallback(TransactionPtr<ConnectionType> pTransaction)
{
...
template <typename ConnectionType>
uint32_t DatabaseWorkerPool<ConnectionType>::OpenConnections(EConnectionTypeIndex connType,
                                                             uint8_t              connectionCount)
{
    for (uint8_t i = 0; i < connectionCount; ++i)
    {
        ...
        switch (connType)
        {
            ...
            default:
                Assert(false,
                       std::format("未知的连接类型索引:{}，请检查配置，服务器退出...",
                                   Util::ToUnderlying(connType)));
                break;
        };
