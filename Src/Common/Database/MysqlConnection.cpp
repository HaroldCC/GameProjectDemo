...
#include "Common/Util/Performance.hpp"
...
IMySqlConnection::IMySqlConnection(ProducerConsumerQueue<ISqlTask *> *queue,
                                   const MySqlConnectionInfo         &connInfo)
    : _connectionInfo(connInfo)
    , _reconnecting(false)
    , _prepareError(false)
...
uint32_t IMySqlConnection::Open()
{
    ...
    if (nullptr == _mysql)
    {
        ...
        Log::Error("连接数据库失败:{} [host:{}, user:{}, password:{}, database:{}, port:{}]",
                   mysql_error(_mysql),
                   _connectionInfo._host,
                   _connectionInfo._user,
                   _connectionInfo._password,
...

    {
        ...
        if (mysql_stmt_bind_param(pMySqlStmt, pMysqlBind))
        {
            ...
            Log::Debug("sql:[{}], 绑定参数错误：{}:{}",
                       pMySqlPreparedStmt->GetSqlString(),
                       errcode,
...
MySqlPreparedStatement *IMySqlConnection::GetPrepareStatement(uint32_t index)
{
    ...
    if (nullptr == pMySqlPreparedStmt)
    {
        Log::Error("获取数据库：{} 预处理语句索引 {} 失败，连接方式：{}",
                   _connectionInfo._database,
                   index,
                   (Util::ToUnderlying(_connectType) & Util::ToUnderlying(ConnectionType::ASYNC)) ? "异步"
                                                                                                  : "同步");
    }
...
bool IMySqlConnection::Query(PreparedStatementBase   *stmt,
                             ...
                             uint32_t                &fieldCount)
{
    ...
    Assert(nullptr != pMySqlPreparedStmt,
           std::format("尝试获取索引为 {} 的预处理语句错误，请排查原因", index));
